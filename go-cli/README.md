...
boru-driven command-line client **and** interactive REPL for the CompanyEnrich
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/company-enrich-cli)
make build

# 2. See usage (words, entities, env vars)
./company-enrich-cli --help

# 3. Provide credentials once, via the environment
export COMPANY_ENRICH_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./company-enrich-cli load 1 company_enrichment            # {id:1} shorthand
./company-enrich-cli load '{id:1}' company_enrichment       # explicit match map

# 5. Override the API base URL for a single call
COMPANY_ENRICH_BASE=https://api.example.com ./company-enrich-cli load 1 company_enrichment

# 6. No arguments -> interactive REPL
./company-enrich-cli
company-enrich> load 1 company_enrichment
company-enrich> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/company-enrich-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/company-enrich-cli load 1 company_enrichment
   ```
...
```sh
./company-enrich-cli load 1 company_enrichment          # scalar shorthand for {id:1}
./company-enrich-cli load '{id:1}' company_enrichment     # explicit match map
```
...
./company-enrich-cli load 1 company_enrichment
```
...
Run with no arguments to open a REPL (prompt `company-enrich>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `company_enrichment`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/company-enrich-cli`. |
...
(`./company-enrich-cli <expr>`) and interactively (the REPL), and expressions compose the
