...
module CompanyEnrichUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = CompanyEnrichResponse.new({})
    ...
    if fetch_err
      ...
      response = CompanyEnrichResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = CompanyEnrichResponse.new(fetched)
    else
