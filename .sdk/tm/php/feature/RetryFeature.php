...
// CompanyEnrich SDK retry feature
...
class CompanyEnrichRetryFeature extends CompanyEnrichBaseFeature
{
    ...
    public function init(CompanyEnrichContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (CompanyEnrichContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(CompanyEnrichContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(CompanyEnrichContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
