...
// CompanyEnrich SDK feature test
//
...
class FtCtrl extends CompanyEnrichControl
{
...
class FtClient
{
    ...
    private CompanyEnrichUtility $utility;

    public function __construct(CompanyEnrichUtility $utility, array $options)
    {
    ...
    public function get_utility(): CompanyEnrichUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (CompanyEnrichContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public CompanyEnrichContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (CompanyEnrichContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = CompanyEnrichConfig::make_feature($name);
    ...
    private static function buildUrl(CompanyEnrichSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof CompanyEnrichError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new CompanyEnrichResponse($fetched);
        }
    ...
    private function fail(CompanyEnrichContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (CompanyEnrichContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
