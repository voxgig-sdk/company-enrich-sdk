# CompanyEnrich Ruby SDK



The Ruby SDK for the CompanyEnrich API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.CompanyEnrichment` — with named operations (`list`/`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```ruby
client = CompanyEnrichSDK.test(testopts, sdkopts)
```
...
raise a `CompanyEnrichError` (a `StandardError` subclass), so wrap
...
```
rb/
├── CompanyEnrich_sdk.rb       -- Main SDK module
...
The main module (`CompanyEnrich_sdk`) exports the SDK class
