...
class CompanyEnrichmentEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = company_enrichment_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "company_enrichment." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    company_enrichment_ref01_data = nil
    if company_enrichment_ref01_data_raw.length > 0
...
def company_enrichment_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "company_enrichment", "CompanyEnrichmentTestData.json")
