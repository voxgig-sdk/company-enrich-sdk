...
class SimilarDirectTest < Minitest::Test
  def test_direct_list_similar
    setup = similar_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-similar", setup[:live] ? "live" : "unit")
    if _should_skip
...
def similar_direct_setup(mockres)
