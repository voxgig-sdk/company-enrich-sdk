...
class CompanySearchDirectTest < Minitest::Test
  def test_direct_list_company_search
    setup = company_search_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-company_search", setup[:live] ? "live" : "unit")
    if _should_skip
...
def company_search_direct_setup(mockres)
