...
require_relative 'CompanyEnrich_types'
...
class CompanyEnrichSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = CompanyEnrichHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = CompanyEnrichHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, CompanyEnrichFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    CompanyEnrichUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = CompanyEnrichHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = CompanyEnrichHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = CompanyEnrichHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => CompanyEnrichError.new(
        "#{op}_allow",
        "CompanyEnrichSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = CompanyEnrichHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = CompanyEnrichError.new(
        "graphql_error", "CompanyEnrichSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = CompanyEnrichSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
