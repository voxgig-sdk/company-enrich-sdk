# CompanyEnrich TypeScript SDK

The TypeScript SDK for the CompanyEnrich API. Provides a type-safe, entity-oriented interface with full async/await support.
...
```ts
const client = new CompanyEnrichSDK({ apikey: '...' })
...
| `tester(testopts?, sdkopts?)` | `CompanyEnrichSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `CompanyEnrichSDK.test(testopts?, sdkopts?)` | `CompanyEnrichSDK` | Create a test-mode client. |
...
company-enrich/
├── src/
│   ├── CompanyEnrichSDK.ts        # Main SDK class
