...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.COMPANYENRICH_TEST_LIVE ||
    'TRUE' === process.env.COMPANYENRICH_TEST_OVERRIDE
  ) {
  ...
  m.COMPANYENRICH_TEST_EXPLAIN = process.env.COMPANYENRICH_TEST_EXPLAIN || m.COMPANYENRICH_TEST_EXPLAIN
