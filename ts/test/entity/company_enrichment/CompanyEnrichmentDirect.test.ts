...
describe('CompanyEnrichmentDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new CompanyEnrichSDK({
  ...
  test('direct-load-company_enrichment', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-company_enrichment', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['COMPANY_ENRICH_TEST_COMPANY_ENRICHMENT_ENTID']
