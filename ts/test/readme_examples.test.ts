...
//     (\`new CompanyEnrichSDK(...)\` / \`CompanyEnrichSDK.test(...)\` become
//     \`CompanyEnrichSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'CompanyEnrichSDK'
...
const TEST_SEED = {"entity":{"company_enrichment":{"test01":{"id":"test01"}},"company_search":{"test01":{"id":"test01"}},"similar":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new CompanyEnrichSDK(...)\` and any
// \`CompanyEnrichSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+CompanyEnrichSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/CompanyEnrichSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
