...
type Args struct {
	Entity string         `json:"entity" jsonschema:"company_enrichment | company_search | similar"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.CompanyEnrichSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "company-enrich_list",
		Description: "List records from CompanyEnrich. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "company-enrich_load",
		Description: "Load a single record from CompanyEnrich. " +
...
func runOp(client *sdk.CompanyEnrichSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.CompanyEnrichSDK, name string) (sdk.CompanyEnrichEntity, error) {
	switch strings.ToLower(name) {
	case "company_enrichment":
		return client.CompanyEnrichment(nil), nil
	case "company_search":
		return client.CompanySearch(nil), nil
	case "similar":
