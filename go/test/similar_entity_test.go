...
func TestSimilarEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Similar(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Similar(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		similarRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.similar")))
		...
		similarRef01Match := map[string]any{}

		similarRef01ListResult, err := similarRef01Ent.List(similarRef01Match, nil)
		if err != nil {
			t.Fatalf("list failed: %v", err)
		}
		_, similarRef01ListOk := similarRef01ListResult.([]any)
		if !similarRef01ListOk {
			t.Fatalf("expected list result to be an array, got %T", similarRef01ListResult)
		}
...
func similarBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("COMPANY_ENRICH_TEST_SIMILAR_ENTID")
	...
	idmapResolved := core.ToMapAny(env["COMPANY_ENRICH_TEST_SIMILAR_ENTID"])
