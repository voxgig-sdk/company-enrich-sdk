...
func TestCompanySearchEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.CompanySearch(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.CompanySearch(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		companySearchRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.company_search")))
...
func company_searchBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("COMPANY_ENRICH_TEST_COMPANY_SEARCH_ENTID")
	...
	idmapResolved := core.ToMapAny(env["COMPANY_ENRICH_TEST_COMPANY_SEARCH_ENTID"])
