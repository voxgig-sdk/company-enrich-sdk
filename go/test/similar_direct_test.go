...
func TestSimilarDirect(t *testing.T) {
	t.Run("direct-list-similar", func(t *testing.T) {
		setup := similarDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-similar", _mode); _shouldSkip {
			if _reason == "" {
