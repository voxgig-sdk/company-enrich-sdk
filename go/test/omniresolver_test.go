...
import (
	...
	omni "github.com/voxgig-sdk/company-enrich-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.CompanyEnrichError); is {
				out := map[string]any{
					"name":    "CompanyEnrichError",
...
func omniCtx(arg any, client *sdk.CompanyEnrichSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.CompanyEnrichSDK); is {
			client = live
