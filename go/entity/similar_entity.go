...
func NewSimilarEntity(client *core.CompanyEnrichSDK, entopts map[string]any) *SimilarEntity {
	if entopts == nil {
		entopts = map[string]any{}
	}
...
func (e *SimilarEntity) GetName() string { return e.name }
...
func (e *SimilarEntity) MarkDeleted() {
	e.deleted = true
}


// Deleted reports whether a successful Remove has resolved on this instance.
func (e *SimilarEntity) Deleted() bool {
	return e.deleted
}


func (e *SimilarEntity) Make() core.Entity {
	opts := map[string]any{}
	for k, v := range e.entopts {
		opts[k] = v
	}
	return NewSimilarEntity(e.client, opts)
}

func (e *SimilarEntity) Data(args ...any) any {
...
func (e *SimilarEntity) Match(args ...any) any {
	if len(args) > 0 && args[0] != nil {
...
// argument it returns the current data as an Similar; with an argument it
// sets the data and returns the stored value. It delegates to the untyped Data
// (identical runtime) and converts at the typed boundary.
func (e *SimilarEntity) DataTyped(data ...Similar) Similar {
	if len(data) > 0 {
		return typedFrom[Similar](e.Data(asMap(data[0])))
	}
	return typedFrom[Similar](e.Data())
}

// MatchTyped mirrors DataTyped for the entity's match filter. The match is a
// partial of the entity, so it round-trips through Similar (all fields
// optional at the wire level).
func (e *SimilarEntity) MatchTyped(match ...Similar) Similar {
	if len(match) > 0 {
		return typedFrom[Similar](e.Match(asMap(match[0])))
	}
	return typedFrom[Similar](e.Match())
}
...
func (e *SimilarEntity) Stream(action string, args map[string]any, callopts map[string]any) <-chan any {
	out := make(chan any)
...
func (e *SimilarEntity) Load(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("load", e.name)
}



func (e *SimilarEntity) List(reqmatch map[string]any, ctrl map[string]any) (any, error) {
	utility := e.utility
...
// SimilarListMatch and returns []Similar. It delegates to the untyped
// List (identical runtime) and converts at the typed boundary.
func (e *SimilarEntity) ListTyped(reqmatch SimilarListMatch, ctrl map[string]any) ([]Similar, error) {
	res, err := e.List(asMap(reqmatch), ctrl)
	if err != nil {
		return nil, err
	}
	return typedSliceFrom[Similar](res), nil
}



func (e *SimilarEntity) Create(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("create", e.name)
}


func (e *SimilarEntity) Update(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("update", e.name)
}


func (e *SimilarEntity) Remove(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("remove", e.name)
}


func (e *SimilarEntity) runOp(ctx *core.Context, postDone func()) (any, error) {
	utility := e.utility
