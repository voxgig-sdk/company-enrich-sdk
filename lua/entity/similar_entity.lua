...
local SimilarEntity = {}
SimilarEntity.__index = SimilarEntity


function SimilarEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SimilarEntity)
  self._name = "similar"
  self._client = client
...
function SimilarEntity:get_name()
  return self._name
end


function SimilarEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SimilarEntity.new(self._client, opts)
end
...
function SimilarEntity:mark_deleted()
  self._deleted = true
end


function SimilarEntity:deleted()
  return true == self._deleted
end


function SimilarEntity:data_set(args)
  if args ~= nil then
...
function SimilarEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SimilarEntity:match_set(args)
  if args ~= nil then
...
function SimilarEntity:match_get()
...
function SimilarEntity:stream(action, args, callopts)
...
function SimilarEntity:_run_op(ctx, post_done)
  local utility = self._utility
