...
local CompanyEnrichmentEntity = {}
CompanyEnrichmentEntity.__index = CompanyEnrichmentEntity


function CompanyEnrichmentEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CompanyEnrichmentEntity)
  self._name = "company_enrichment"
  self._client = client
...
function CompanyEnrichmentEntity:get_name()
  return self._name
end


function CompanyEnrichmentEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CompanyEnrichmentEntity.new(self._client, opts)
end
...
function CompanyEnrichmentEntity:mark_deleted()
  self._deleted = true
end


function CompanyEnrichmentEntity:deleted()
  return true == self._deleted
end


function CompanyEnrichmentEntity:data_set(args)
  if args ~= nil then
...
function CompanyEnrichmentEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CompanyEnrichmentEntity:match_set(args)
  if args ~= nil then
...
function CompanyEnrichmentEntity:match_get()
...
function CompanyEnrichmentEntity:stream(action, args, callopts)
...
function CompanyEnrichmentEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return CompanyEnrichmentEntity
