...
local CompanySearchEntity = {}
CompanySearchEntity.__index = CompanySearchEntity


function CompanySearchEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CompanySearchEntity)
  self._name = "company_search"
  self._client = client
...
function CompanySearchEntity:get_name()
  return self._name
end


function CompanySearchEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CompanySearchEntity.new(self._client, opts)
end
...
function CompanySearchEntity:mark_deleted()
  self._deleted = true
end


function CompanySearchEntity:deleted()
  return true == self._deleted
end


function CompanySearchEntity:data_set(args)
  if args ~= nil then
...
function CompanySearchEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CompanySearchEntity:match_set(args)
  if args ~= nil then
...
function CompanySearchEntity:match_get()
...
function CompanySearchEntity:stream(action, args, callopts)
...
function CompanySearchEntity:_run_op(ctx, post_done)
  local utility = self._utility
