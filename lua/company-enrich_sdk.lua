...
local CompanyEnrichSDK = {}
CompanyEnrichSDK.__index = CompanyEnrichSDK
...
CompanyEnrichSDK._make_feature = _make_feature


function CompanyEnrichSDK.new(options)
  local self = setmetatable({}, CompanyEnrichSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function CompanyEnrichSDK:options_map()
...
function CompanyEnrichSDK:get_utility()
  return Utility.copy(self._utility)
end


function CompanyEnrichSDK:get_root_ctx()
  return self._rootctx
end


function CompanyEnrichSDK:prepare(fetchargs)
...
function CompanyEnrichSDK:direct(fetchargs)
...
-- by passing it.
function CompanyEnrichSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function CompanyEnrichSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "CompanyEnrichSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function CompanyEnrichSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = CompanyEnrichSDK.new(sdkopts)
  sdk.mode = "test"
