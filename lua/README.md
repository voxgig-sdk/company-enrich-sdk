# CompanyEnrich Lua SDK



The Lua SDK for the CompanyEnrich API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:CompanyEnrichment()` — each with the same small set of operations (`list`, `load`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
Check `err` first (it is non-`nil` on failure), then use `value`:
    ...
    local company_enrichment, err = client:CompanyEnrichment():load()
    if err then error(err) end
...
```
lua/
├── company-enrich_sdk.lua    -- Main SDK module
...
The main module (`company-enrich_sdk`) exports the SDK constructor
