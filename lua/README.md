...
The Lua SDK for the CompanyEnrich API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:CompanyEnrichment(nil):load({ id = "example_id" }, nil)
...
local result, err = client:CompanyEnrich(nil):load(
  { id = "test01" }, nil
)
