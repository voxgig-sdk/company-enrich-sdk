# CompanyEnrich Python SDK

The Python SDK for the CompanyEnrich API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.CompanyEnrichment(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.CompanyEnrich(None).load(
    {"id": "test01"}, None
)
...
```
py/
├── companyenrich_sdk.py         -- Main SDK module
