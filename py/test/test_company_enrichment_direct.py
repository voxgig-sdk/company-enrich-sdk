...
class TestCompanyEnrichmentDirect:

    def test_should_direct_load_company_enrichment(self):
        setup = _company_enrichment_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-company_enrichment", "live" if setup["live"] else "unit")
        if _skip:
...
def _company_enrichment_direct_setup(mockres):
