...
class TestSimilarDirect:

    def test_should_direct_list_similar(self):
        setup = _similar_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-similar", "live" if setup["live"] else "unit")
        if _skip:
...
def _similar_direct_setup(mockres):
