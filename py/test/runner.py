...
class CompanyEnrichTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            CompanyEnrichTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = CompanyEnrichTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = CompanyEnrichTestRunner.getenv("COMPANY_ENRICH_TEST_LIVE")
        override = CompanyEnrichTestRunner.getenv("COMPANY_ENRICH_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = CompanyEnrichTestRunner.getenv(key)
        ...
        explain = CompanyEnrichTestRunner.getenv("COMPANY_ENRICH_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if CompanyEnrichTestRunner._test_control is not None:
            return CompanyEnrichTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                CompanyEnrichTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            CompanyEnrichTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = CompanyEnrichTestRunner.load_test_control()
...
def load_env_local():
    CompanyEnrichTestRunner.load_env_local()


def env_override(m):
    return CompanyEnrichTestRunner.env_override(m)


def entity_data(v):
    return CompanyEnrichTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return CompanyEnrichTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return CompanyEnrichTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return CompanyEnrichTestRunner.load_test_control()


def live_client_options():
    return CompanyEnrichTestRunner.live_client_options()


def live_delay_ms():
    return CompanyEnrichTestRunner.live_delay_ms()
