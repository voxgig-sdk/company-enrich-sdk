...
class TestCompanySearchDirect:

    def test_should_direct_list_company_search(self):
        setup = _company_search_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-company_search", "live" if setup["live"] else "unit")
        if _skip:
...
def _company_search_direct_setup(mockres):
