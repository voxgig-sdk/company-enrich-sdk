...
class CompanyEnrichmentDirectTest extends TestCase
{
    public function test_direct_load_company_enrichment(): void
    {
        $setup = company_enrichment_direct_setup(["id" => "direct01"]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-load-company_enrichment", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
