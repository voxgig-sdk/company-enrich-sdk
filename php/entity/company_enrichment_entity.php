...
// CompanyEnrich SDK CompanyEnrichment entity
...
class CompanyEnrichmentEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new CompanyEnrichmentEntity($this->_client, $opts);
    }

    /**
     * @param CompanyEnrichment|array $args CompanyEnrichment data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = CompanyEnrichHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return CompanyEnrichment|array The current CompanyEnrichment data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of CompanyEnrichment fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = CompanyEnrichHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of CompanyEnrichment fields).
     */
    ...
    /**
     * Load a single CompanyEnrichment.
     *
     * @param CompanyEnrichmentLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed CompanyEnrichmentLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return CompanyEnrichment|array The loaded CompanyEnrichment as an assoc-array at the
     *   SDK boundary; throws CompanyEnrichError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = CompanyEnrichHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
