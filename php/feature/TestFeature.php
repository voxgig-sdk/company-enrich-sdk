...
// CompanyEnrich SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class CompanyEnrichTestFeature extends CompanyEnrichBaseFeature
{
    ...
    public function init(CompanyEnrichContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (CompanyEnrichContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = CompanyEnrichParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (CompanyEnrichContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(CompanyEnrichContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = CompanyEnrichParam::call($ctx, $k);
